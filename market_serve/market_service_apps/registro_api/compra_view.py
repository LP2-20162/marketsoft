...
class MiPermission(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        perms = ('registro.list_compra',)  # cambie aqui el permiso
...
class CompraViewSet(ModelPagination, viewsets.ModelViewSet):
    ...
    def reporte_Compraes(self, request, *args, **kwargs):
        lista = []
        pre_query = self.get_queryset().values()
        for x in pre_query:
            lista.append([x['trabajador'], x['lote']])
        print(lista)
        #data = Compra.objects.pdf(lista, 'mi primer reporte')
