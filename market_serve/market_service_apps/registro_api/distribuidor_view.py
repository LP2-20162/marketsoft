...
from market_service_apps.registro.models.distribuidor import Distribuidor
...
class MiPermission(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        perms = ('registro.list_distribuidor',)  # cambie aqui el permiso
...
class DistribuidorViewSet(ModelPagination, viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        #    return Distribuidor.objects.all()
    ...
    def reporte_distribuidors(self, request, *args, **kwargs):
        lista = []
        pre_query = self.get_queryset().values()
        for x in pre_query:
            lista.append([x['nombre'], x['direccion']])
        print(lista)
        #data = Distribuidor.objects.pdf(lista, 'mi primer reporte')
